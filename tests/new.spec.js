...
test("this is my test case", async ({ page }) => {
  ...
  await page
    .getByRole("link", { name: "Register for free" })
    .waitFor({ state: "visible" });
  await page
    .getByRole("textbox", { name: "Enter your active Email ID / Username" })
    .fill("syedcs36@gmail.com");
  await expect(
    page.getByRole("link", { name: "Register for free" }),
  ).toHaveText("Register for free"); //.toBeVisible(); //
